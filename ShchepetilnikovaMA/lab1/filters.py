...
#Базовый класс для всех фильтров изображений
class Filter:    
...
class ResizeImage(Filter):    
    ...
    def apply(self, image, parameters):
        ...
        if scale_percent is not None and scale_percent < 100:
            interpolation = cv.INTER_AREA  #Для уменьшения
        elif scale_percent is not None and scale_percent > 100:
            interpolation = cv.INTER_CUBIC  #Для увеличения
        else:
            interpolation = cv.INTER_LINEAR  #По умолчанию
...
class SepiaFilter(Filter):    
    ...
    def apply(self, image, parameters):
        ...
        #Смешивание с единичной матрицей для регулировки интенсивности
...
class VignetteFilter(Filter):    
    ...
    def apply(self, image, parameters):
        ...
        result = result * mask[:, :, np.newaxis]  #Применение маски к каждому каналу
...
class PixelateFilter(Filter):   
    ...
    def apply(self, image, parameters):
        ...
        #Извлечение области для пикселизации
        area = result[y:y_end, x:x_end]
        block_size = max(2, min(block_size, min(actual_width, actual_height) // 2))
        # Уменьшение и увеличение размера для создания эффекта пикселизации
...
class RectBorder(Filter):    
    ...
    def apply(self, image, parameters):
        ...
        # Применение рамки ко всем сторонам
...
class FrameFilter(Filter):    
    ...
    def apply(self, image, parameters):
        ...
        if frame_type == "circle":
            ...
            points = np.array([
                [img_width // 2, 0],           #Верх
                [img_width, img_height // 2],  #Право
                [img_width // 2, img_height],  #Низ
                [0, img_height // 2]           #Лево
            ], dtype=np.int32)
...
class BliksFilter(Filter):    
    ...
    def apply(self, image, parameters):
        ...
        if cy is None:
            cy = img_height // 2
        #Создание сетки расстояний от центра
        y, x = np.ogrid[:img_height, :img_width]
        distance = np.sqrt((x - cx)**2 + (y - cy)**2)
        max_dist = np.sqrt(cx**2 + cy**2)
        #Создание эффекта блика
        flare = np.exp(-2 * distance / max_dist) * intensity
        flare = np.dstack([flare, flare, flare])  #Применение ко всем каналам
