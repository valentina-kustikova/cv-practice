...
- SSD MobileNet v2 (TensorFlow → OpenCV)
...
YOLO предсказывает координаты в формате:

> центр объекта + ширина и высота рамки

Чтобы получить координаты рамки, как принято в компьютерном зрении, выполняется преобразование к формату:

> левый верхний угол + ширина и высота
...
Поэтому размеры и координаты рамок пересчитываются:

> из размеров входа сети → к размерам исходного изображения
...
Координаты рамок уже заданы в формате:
...
### 2.5. Постобработка NanoDet — подробное описание алгоритма
