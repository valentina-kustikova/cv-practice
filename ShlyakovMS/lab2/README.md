...
3. Для каждого кадра:
   ...
   - Сравнивает полученные детекции с аннотированными объектами по IoU и вычисляет метрики кадра (TPR и FDR).
...
4. По завершении:
   ...
   - Сохраняет краткий текстовый отчёт с усреднёнными метриками.
...
- Детекция считается TP, если существует неподвязанная аннотация того же класса с IoU ≥ 0.5.
- На уровне кадра вычисляются:
  - TPR — доля найденных объектов от числа объектов в разметке кадра.
  - FDR — доля ложных срабатываний среди всех детекций кадра.
- По завершении усредняются метрики по всем кадрам и сохраняется отчёт.
