...
| Модель | TPR (Полнота) | FDR (Ложные сраб.) | Примечание |
| :--- | :---: | :---: | :--- |
| **YOLOv8** | 0.937 | 0.265 | Максимальная чувствительность |
| **Faster R-CNN** | 0.702 | 0.069 | Минимальное число ошибок |
| **NanoDet-Plus** |  0.863 | 0.167 | Баланс точности и скорости |

### Анализ результатов

На основе полученных метрик можно сделать следующие наблюдения:

1.  **YOLOv8** продемонстрировала **наивысшую полноту (TPR = 0.937)**. Модель успешно обнаруживает подавляющее большинство транспортных средств в кадре. Однако платой за высокую чувствительность стал высокий уровень ложных срабатываний (**FDR = 0.265**). Это означает, что примерно каждая четвертая детекция является ошибочной (ложный объект или неточный бокс).
2.  **Faster R-CNN** показала себя как самая "осторожная" модель. У неё **минимальный уровень ошибок (FDR = 0.069)** — если модель что-то находит, это с высокой вероятностью является машиной. Однако из-за такой избирательности она пропустила около 30% объектов (**TPR = 0.702**), что является худшим показателем среди трех участников.
3.  **NanoDet-Plus** показала сбалансированный результат. Она значительно превосходит Faster R-CNN по способности находить объекты (**TPR = 0.863**) и при этом генерирует в 1.5 раза меньше ложных срабатываний, чем YOLOv8 (**FDR = 0.167**).

### Выводы

Выбор оптимальной модели зависит от приоритетов прикладной задачи:

*   Если критически важно **не пропустить ни одного объекта** (например, в задачах видеонаблюдения или подсчета трафика в реальном времени), лучшим выбором является **YOLOv8**.
*   Для систем, где **цена ложного срабатывания высока** (например, автоматическая выписка штрафов, где нельзя ошибиться), предпочтительнее использовать **Faster R-CNN**, так как она обеспечивает наивысшую точность предсказаний (Precision).
*   **NanoDet-Plus** представляет собой "золотую середину", обеспечивая достойное качество детекции при легковесной архитектуре, что делает её идеальным кандидатом для запуска на мобильных и встраиваемых устройствах.
