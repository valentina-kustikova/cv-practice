...
**Алгоритм:**
К изображению применяется эффект блика путём наложения готовой текстуры блика, которая автоматически масштабируется под размер изображения. Параметр интенсивности управляет прозрачностью и силой наложения текстуры.

**Формула:**
```bash
I_new = (1 - α) * I_orig + α * I_blik
```
где `I_blik` — текстура блика, а `α` (`intensity`) — сила наложения.

**Пример запуска:**
```bash
python script.py photo.jpg lens_flare --params 0.7
```
...
**Алгоритм:**
К изображению применяется эффект акварельной бумаги путём наложения готовой текстуры, которая автоматически масштабируется под размер изображения. Итоговое изображение формируется как взвешенное среднее между исходным изображением и текстурой.

**Формула:**
```bash
I_new = (1 - α) * I_orig + α * I_texture
```
где `I_texture` — текстура, а `α` (`strength`) — сила наложения.

**Пример запуска:**
```bash
python script.py cat.jpg watercolor --params 0.3
```
