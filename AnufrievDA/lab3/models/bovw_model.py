...
class BoVWClassifier:
    ...
    def _init_feature_extractor(self):
        # Ограничиваем число точек до 500
        ...
        for i, path in enumerate(image_paths):
            try:
                ...
                if des is not None and len(des) > 0:
                    # ВАЖНО: Конвертируем сразу в float64 во избежание ошибок типов
    ...
    def fit(self, image_paths: List[str], labels: List[int]):
        ...
        print(f"Обучение кластеризатора (словаря) на {len(all_des)} векторах...")
        # Явно указываем float64
