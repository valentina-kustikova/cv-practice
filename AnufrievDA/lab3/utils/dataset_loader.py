...
def parse_class_from_path(path: str) -> int:
    ...
    # Если не нашли в имени, пробуем стандартный фолбек (если папки названы 0, 1, 2)
    # Но согласно ТЗ приоритет у префиксов выше
    return -1
    ...
    for root, dirs, files in os.walk(data_root):
        for file in files:
            if file.lower().endswith(valid_exts):
                ...
                lbl = parse_class_from_path(file) # или full_path
                if lbl == -1:
                    # Попробуем определить по имени папки
