...
def main():
    ...
    try:
        if filter_name == 'resize':
            ...
            result_image = filters.pixelate_area(original_image, x=w//4, y=h//4, w=w//2, h=h//2, pixel_size=args.pixel_size)
        elif filter_name == 'simple_frame':
            try:
                b, g, r = map(int, args.color.split(','))
                frame_color = (b, g, r)
            except ValueError:
                print("Ошибка: неверный формат цвета. Используйте B,G,R, например, '0,0,255'.")
                return
