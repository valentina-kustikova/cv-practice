...
def image_pixelate(img, coords, pixel_size):
    """
    Функция пикселизации выделенной прямоугольной области изображения

    :param img: Исходное изображение
    :param coords: Координаты области для пикселизации
...
if __name__ == '__main__':
    ...
    orig_win_name = "Original"
    cv.imshow(orig_win_name, input_image)
    ...
    match filter_type:
        ...
        case "pixelate":
            coords = cv.selectROI(orig_win_name, input_image, showCrosshair=False, printNotice=False)
            output_image = image_pixelate(input_image, coords, args.pixel_size)
