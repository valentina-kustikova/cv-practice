...
# 5. Пикселизировать область (по координатам)
python main.py test_image.jpg pixelate --x 100 --y 100 --width 300 --height 300 --pixel_size 20

# 5b. Интерактивная пикселизация (выбор области мышью)
python main.py test_image.jpg pixelate_interactive --pixel_size 15
...
### 4b. Функция интерактивной пикселизации (`interactive_pixelate`)

**Описание:**
Интерактивный режим пикселизации с выбором области мышью. Использует callback-функцию для обработки событий мыши.

**Алгоритм:**
1. Создаем словарь состояния (`state`) для хранения:
   - Координаты начальной и конечной точек выделения
   - Флаг рисования
   - Копии оригинального и текущего изображения
   - Размер пикселя
2. Определяем callback-функцию `mouse_callback` для обработки событий мыши:
   - `EVENT_LBUTTONDOWN`: запоминаем начальную точку, активируем режим рисования
   - `EVENT_MOUSEMOVE`: обновляем конечную точку и показываем прямоугольник выделения
   - `EVENT_LBUTTONUP`: применяем пикселизацию к выбранной области
3. Создаем окно OpenCV и регистрируем callback через `cv2.setMouseCallback()`
4. В цикле обрабатываем нажатия клавиш:
   - `ESC`: выход без сохранения
   - `s`: сохранить результат и выйти
...
- `r`: сбросить все изменения
- `s`: сохранить результат и выйти
- `ESC`: выйти без сохранения

**Используемые базовые операции:**
- `cv2.namedWindow()` для создания окна
- `cv2.setMouseCallback()` для регистрации callback-функции
- `cv2.imshow()` для отображения изображения
- `cv2.rectangle()` для отрисовки прямоугольника выделения
- `cv2.waitKey()` для обработки нажатий клавиш
