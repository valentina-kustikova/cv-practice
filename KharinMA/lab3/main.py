...
from classifier.efficientnet_classifier import EfficientNetClassifier
...
def parse_args():
    ...
    optional_args.add_argument('--algorithm', type=str, choices=['bow', 'efficientnet', 'both'],
    ...
    optional_args.add_argument('--learning_rate', type=float, default=0.001,
                               help='Скорость обучения для EfficientNet')
    optional_args.add_argument('--batch_size', type=int, default=16,
                               help='Размер батча для EfficientNet')
    optional_args.add_argument('--epochs', type=int, default=20,
                               help='Количество эпох для EfficientNet')
    optional_args.add_argument('--model_dir', type=str, default=None,
                               help='Директория для загрузки/сохранения модели')
...
def visualize_sift_features(args):
    ...
    if result_image is not None:
        # Показываем изображение, если не указан путь для сохранения
        if args.visualize_output is None:
            # cv2.imshow might not work in some environments, but we keep it as per reference
...
def main():
    ...
    # Если запущен режим визуализации, выполняем его и выходим
    if args.visualize:
        visualize_sift_features(args)
        return

    # Если это не режим визуализации, проверяем наличие обязательных аргументов
    ...
    # Обработка EfficientNet классификатора
    if args.algorithm in ['efficientnet', 'both']:
        print("\n=== EfficientNet классификатор ===")
        if args.algorithm == 'efficientnet' and args.model_dir is not None:
            eff_model_dir = args.model_dir
        else:
            eff_model_dir = os.path.join(SCRIPT_DIR, 'efficientnet_model')
        eff_classifier = EfficientNetClassifier(
            model_dir=eff_model_dir,
            learning_rate=args.learning_rate,
            batch_size=args.batch_size,
            epochs=args.epochs
        )
        eff_accuracy = train_and_test_classifier(eff_classifier, args)
        if eff_accuracy is not None:
            results['EfficientNet'] = eff_accuracy
