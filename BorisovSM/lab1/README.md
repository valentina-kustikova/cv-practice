...
## 4. Функция `_pixelate_region(image, x, y, w, h, block)` и интерактивный вариант `_roi_pixelate(image, block)`
...
- `x, y` — координаты левого верхнего угла области (для `_pixelate_region`)
- `w, h` — ширина и высота области (для `_pixelate_region`)
