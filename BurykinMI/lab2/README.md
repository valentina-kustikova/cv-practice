...
| **YOLO** | ~0.972        | ~0.186               | Низкая (на CPU) | Высочайшая полнота (находит почти всё), часто находит реальные объекты, которые не были выделены разметкой (из-за этого FDR выше) и требует больше ресурсов. |
