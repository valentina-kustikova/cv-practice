...
Приложение поддерживает три архитектуры нейронных сетей, реализованные в виде иерархии классов.
...
Облегченная и ускоренная версия архитектуры YOLOv4, предназначенная для работы на устройствах с ограниченными ресурсами.

* **Предобработка:**
    * Аналогична YOLOv3: вход **416x416**.
    * Масштабирование: `1/255.0`.
    * Конвертация каналов SwapRB.
* **Постобработка:**
    * Так же использует NMS для фильтрации рамок.
    * Обучена на датасете COCO (80 классов), фильтруются только транспортные классы.
* **Особенности:** Значительно быстрее YOLOv3, при этом на данной задаче показывает сравнимое качество (TPR), немного уступая в стабильности рамок.
...
|-----------------|---------------|----------------------|-----------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **SSD**         | ~0.746        | ~0.032               | Высокая         | Высокая точность (Precision), мало ложных срабатываний, но пропускает сложные объекты.                                                                       |
| **YOLOv3**      | ~0.972        | ~0.186               | Низкая (на CPU) | Высочайшая полнота (находит почти всё), часто находит реальные объекты, которые не были выделены разметкой (из-за этого FDR выше) и требует больше ресурсов. |
| **YOLOv4-Tiny** | ~0.890        | ~0.082               | Высокая         | Работает гораздо быстрее YOLOv3, при этом имея достаточно высокий TPR.                                                                                       |

**Выводы:**

* **SSD** предпочтительна для систем реального времени на слабых устройствах, где критично отсутствие ложных тревог.
* **YOLOv3** показывает лучшие результаты там, где недопустимо пропустить объект, и есть вычислительные мощности для
  фильтрации результатов.
* **YOLOv4-Tiny** Является золотой серединой между этими моделями, имея высокий TPR, низкий FDR и хорошую скорость работы.
...
**Запуск с моделью YOLOv3:**

```bash
python main.py --model yolo --frames_dir data/MOV03478 --gt_file data/MOV03478/mov03478.txt
```

**Запуск с моделью YOLOv4-Tiny:**

```bash
python main.py --model yolov4-tiny --frames_dir data/MOV03478 --gt_file data/MOV03478/mov03478.txt
```

### Параметры:

* `--model`: выбор архитектуры (`ssd` или `yolo` или `yolov4-tiny`).
