...
Отметим, что время выполнения программы с детектором MobileNet-SSD в раз меньше, чем с YOLOv5s. На фото с детектированными объектами видно, что MobileNet-SSD ошибается в классах, не очень точно формирует рамку (на двух схожих фото рамка на одном и том же объекте расположена по-разному), иногда детектирует всё фото как объект какого-либо класса. YOLOv5s же может ошибиться только в случае, когда на фото множеста объектов или они присутствуют на фото лишь частично. Даже в этих случаях рамка отрисовывается достаточно точно, и с большой вероятностью автомобиль будет правильно детектирован по одному только своему элементу.

## Выводы
- Реализовано единое API для подключения детекторов.
- Ключевая сложность — адаптация постпроцессинга под конкретный формат выхода ONNX/TensorFlow-моделей.
- Точность метрик напрямую зависит от корректности:
  - масштабирования и учёта паддинга,
  - сортировки детекций по confidence,
  - формата классов (background или без).
- Модульная архитектура позволяет легко добавлять новые модели без изменения основного кода (`app.py`).

 Итог: TPR = 0.451, FDR = 0.027