...
|  | TPR | FDR | Время, с |
|---|---|---|---|
| MobileNet-SSD | 0.479 | 0.035 | 117.13 |
| YOLOv5s | 0.972 | 0.179 | 1628.77 |

Отметим, что время выполнения программы с детектором MobileNet-SSD почти в 14 раз меньше, чем с YOLOv5s. На фото с детектированными объектами видно, что MobileNet-SSD ошибается в классах, не очень точно формирует рамку (на двух схожих фото рамка на одном и том же объекте расположена по-разному), иногда детектирует всё фото как объект какого-либо класса. YOLOv5s же может ошибиться только в случае, когда на фото много объектов или они присутствуют на лишь частично. Даже в этих случаях рамка отрисовывается достаточно точно, и с большой вероятностью автомобиль будет правильно детектирован по одному только своему элементу.

Кроме того, FDR у YOLOv5s выше, чем у MobileNet-SSD - выше доля ошибочных положительных предсказаний среди всех объектов, классифицированных правильно.