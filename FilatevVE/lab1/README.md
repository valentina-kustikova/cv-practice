...
**Алгоритм:**
- Определяется позиция блика в нормализованных координатах
- Вычисляется расстояние от каждого пикселя до центра блика
- Создается гауссова маска: `exp(-(distance²) / (2 * radius²))`
- Маска применяется к каждому цветовому каналу с заданной интенсивностью
- Результат обрезается до диапазона [0, 255]
...
**Алгоритм:**
- Генерируется высокочастотный шум с нормальным распределением
- Создается низкочастотный шум путем уменьшения и увеличения случайной матрицы
- Текстура формируется как комбинация высоко- и низкочастотного шума
- Применяется смешивание: `result = image * (1 - intensity) + image * (1 + texture) * intensity`
- Добавляется легкое размытие по Гауссу для смягчения эффекта
