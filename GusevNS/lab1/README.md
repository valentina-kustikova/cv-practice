...
**Алгоритм:**
- Загружается текстура рамки из файла `textures/frame{N}.jpg` в зависимости от типа
- Текстура масштабируется под размер изображения с помощью `resize_image`
- Создается маска на основе цветового различия от белого фона (порог > 30)
- Рамка накладывается на изображение с учетом маски
...
**Алгоритм:**
- Загружается текстура блика из файла `textures/glare.jpg`
- Текстура масштабируется до 1/4 размера изображения
- Вычисляются координаты размещения блика с учетом границ изображения
- Блик добавляется к изображению с заданной интенсивностью (яркость увеличивается)
...
**Алгоритм:**
- Загружается текстура акварельной бумаги из файла `textures/watercolor_paper.jpg`
- Текстура масштабируется под размер изображения с помощью `resize_image`
- Создается маска на основе яркости текстуры (инвертированная нормализованная яркость)
...
Гусев Никита - Практическая работа по курсу "Основы компьютерного зрения"
