...
- **Что показывает**: Когда модель говорит "это собор", насколько часто она права?
- **Высокий precision**: Мало ложных срабатываний

#### Recall (Полнота)
**Формула:** `Recall = TP / (TP + FN)`
- **FN (False Negative)**: Пропущенные положительные
- **Что показывает**: Сколько процентов реальных соборов модель нашла?
...
- Баланс между "быть точным" и "находить всё"
- **F1 = 1.0**: Идеальный баланс

#### Support (Поддержка)
- Количество реальных примеров данного класса в тестовой выборке
- Показывает размер класса для интерпретации метрик

#### Accuracy (Общая точность)
**Формула:** `Accuracy = (TP + TN) / (TP + FP + FN + TN)`
- Доля всех правильных предсказаний (всех классов)
- Процент изображений, которые модель распознала правильно

#### Macro Average (Макро-усреднение)
- Простое среднее арифметическое по всем классам
- **Равный вес** для каждого класса, независимо от размера

#### Weighted Average (Взвешенное усреднение)
- Среднее взвешенное по количеству примеров в каждом классе
- **Больший вес** для классов с большим количеством примеров
- Более репрезентативная метрика при несбалансированных данных