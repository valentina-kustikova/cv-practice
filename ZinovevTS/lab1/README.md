...
Билинейная интерполяция и интерполяция ближайшего соседа  
 а) Билинейная интерполяция: определение целочисленных координат, извлечение значений четырёх соседних пикселей, вычисление усреднения по формуле $f(x,y) ≈ (1-dx)(1-dy)×f(x₁,y₁) + dx(1-dy)×f(x₂,y₁) + (1-dx)dy×f(x₁,y₂) + dx×dy×f(x₂,y₂)$ 
